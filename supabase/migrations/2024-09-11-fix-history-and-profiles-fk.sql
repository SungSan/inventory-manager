-- Ensure movements_view exposes the expected columns for history API and that users/profiles relationship is recognized
...
-- Reinstate user_profiles -> users foreign key and uniqueness if missing
do $$
begin
  if not exists (
...
-- Refresh PostgREST schema cache so new view/constraint are visible immediately
